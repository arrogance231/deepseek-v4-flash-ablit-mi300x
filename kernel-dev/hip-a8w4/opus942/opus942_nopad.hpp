...
struct gmem {
    ...
    OPUS_D void _async_load(OPUS_LDS_ADDR void* dst, int v_os, int s_os = 0, number<i_os> = {}, number<aux> = {}) {
        ...
        dst = reinterpret_cast<OPUS_LDS_ADDR void*>(reinterpret_cast<__UINTPTR_TYPE__>(dst) + i_os + lane_id() * sizeof(type));
        *reinterpret_cast<OPUS_LDS_ADDR type*>(dst) = val;
#elif defined(__gfx940__) || defined(__gfx941__) || defined(__gfx950__)   // CDNA: buffer-load lands directly in LDS (vmem-to-lds-load-insts).
...
struct smem {
    ...
    template<index_t vec = 1, int imm_offset = 0> OPUS_D auto _tr_load(int v_os/* in unit of byte*/) {
        ...
        using type = vector_type<vec>;
        static_assert(sizeof(type) == 8 && sizeof_bits_v<scalar_type> == 16);
        const int col = lane_id() & 15;
        const int base = v_os + imm_offset - (col >> 2) * 1024 - (col & 3) * 8;
        const int p = col & 3, aligned = (col & ~3) * 2;
        auto row0 = _load<4>(base + aligned);
        auto row1 = _load<4>(base + 1024 + aligned);
        auto row2 = _load<4>(base + 2048 + aligned);
        auto row3 = _load<4>(base + 3072 + aligned);
        type out{row0[p], row1[p], row2[p], row3[p]};
        return out;
