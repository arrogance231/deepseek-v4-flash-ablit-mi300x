...
__device__ __forceinline__ float sparse_nan_to_num(float x) {
  const uint32_t bits = __float_as_uint(x);
  const uint32_t magnitude = bits & 0x7fffffff;
  if (magnitude > 0x7f800000) return -INFINITY;
  if (magnitude == 0x7f800000) return bits >> 31 ? -FLT_MAX : FLT_MAX;
  return x;
}
  ...
  if constexpr (step == 3 && !multipleBlocksPerRow && !mergeBlocks) {
    ...
    for (int tile = 0; tile < rowEnd - rowStart;
      ...
      if (relativeIdx < rowEnd - rowStart) {
        ...
        float logit =
            sparse_nan_to_num(logits[stride1 == 1 ? idx : idx * stride1]);
  ...
  if (rowLen <= topK) {
    for (int rowIt = threadIdx.x; rowIt < rowLen;
      ...
      if constexpr (multipleBlocksPerRow) {
        ...
        outLogits[rowIt] = sparse_nan_to_num(logits[rowIt + rowStart]);
      } else {
